...
import { createFigmaCaptureToken } from "@/lib/figma/provenance";
import {
...
export async function GET(
  ...
  if (!linkId) {
    return jsonError("invalid_link", "Figma resource link is required.", 400);
  }
  ...
  try {
    const observation = await readVerifiedFigmaProjectContext({
      supabase,
      projectId,
      linkId,
    });
    const captureToken = createFigmaCaptureToken({
      projectId,
    ...
    if (error instanceof FigmaReadBoundaryError) {
      return jsonError(error.code, error.message, error.status, error.retryAfterSeconds);
    }
    return jsonError(
